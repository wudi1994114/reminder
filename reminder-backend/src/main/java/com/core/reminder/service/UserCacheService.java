...
public class UserCacheService {
    ...
    /**
     * Loads the delivery profile used by notification senders, including an
     * optional WeChat binding from the same persistence context.
     */
    public UserNotificationProfileDto getUserNotificationProfileById(Long userId) {
        ...
        try {
            Optional<WechatUser> wechatUser = wechatUserRepository.findByAppUserId(userId);
            wechatUser.ifPresent(binding -> {
                notificationProfile.setWechatOpenid(binding.getOpenid());
                notificationProfile.setWechatUnionid(binding.getUnionid());
            });
        } catch (RuntimeException exception) {
            log.error("获取用户ID[{}]的微信信息失败", userId, exception);
        }
